...
const Sidebar = ({ isOpen, event, onClose }) => {
  ...
  const handleSaveClick = () => {
    ...
    if (hasConflict.name) {
      toast.error(
        `Conflict detected! ${hasConflict.name} already has an interview scheduled at ${hasConflict.time}.`
      );
      return;
    }
  ...
  return (
    <div
      className={`fixed top-0 right-0 h-full bg-white shadow-lg transform transition-transform duration-300 ease-in-out ${
        isOpen ? "translate-x-0 w-full sm:w-1/3" : "translate-x-full w-0"
      }`}
      style={{ zIndex: 1000 }}
    >
            ...
            <div className="space-y-4">
              ...
              <div className="flex items-center space-x-3">
                ...
                <div>
                  ...
                  <p className="font-medium">
                    {convertTo12HourFormat(event.start.toString())}
                  </p>
                </div>
              </div>
